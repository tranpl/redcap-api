...
namespace Redcap
{
    ...
    public class RedcapApi : IRedcap
    {
        ...
        public RedcapApi(string redcapApiUrl, bool useInsecureCertificates = false)
            : this(redcapApiUrl, new DefaultRedcapTransport(), useInsecureCertificates)
        {
        }

        /// <summary>
        /// Constructor that accepts both a transport abstraction and certificate mode.
        /// </summary>
        /// <param name="redcapApiUrl">Redcap instance URI</param>
        /// <param name="transport">Transport abstraction used to execute requests.</param>
        /// <param name="useInsecureCertificates">Allows use of insecure certificates in HttpClient</param>
        public RedcapApi(string redcapApiUrl, IRedcapTransport transport, bool useInsecureCertificates = false)
        {
            _uri = new Uri(redcapApiUrl);
            _transport = transport ?? throw new ArgumentNullException(nameof(transport));
        ...
        public async Task<string> ExportRecordsAsync(string token, RedcapFormat format = RedcapFormat.json, RedcapDataType redcapDataType = RedcapDataType.flat, string[] records = default, string[] fields = default, string[] forms = default, string[] events = default, RawOrLabel rawOrLabel = RawOrLabel.raw, RawOrLabelHeaders rawOrLabelHeaders = RawOrLabelHeaders.raw, bool exportCheckboxLabel = false, RedcapReturnFormat returnFormat = RedcapReturnFormat.json, bool exportSurveyFields = false, bool exportDataAccessGroups = false, string filterLogic = null, DateTime? dateRangeBegin = default, DateTime? dateRangeEnd = default, CsvDelimiter csvDelimiter = CsvDelimiter.comma, DecimalCharacter decimalCharacter = DecimalCharacter.none, bool exportBlankForGrayFormStatus = false, bool combineCheckboxOptions = false, CancellationToken cancellationToken = default, long timeOutSeconds = 100)
        {
            try
            {
                ...
                {
                    payload.Add("combineCheckboxOptions", combineCheckboxOptions.ToString());
                }
        ...
        public async Task<string> DeleteRecordsAsync(string token, string[] records, int? arm, CancellationToken cancellationToken = default, long timeOutSeconds = 100, bool deleteLogging = false)
        {
            try
            {
                ...
                if(deleteLogging)
                {
                    payload.Add("delete_logging", deleteLogging.ToString());
                }
                return await this.SendPostRequestAsync(payload, _uri, cancellationToken: cancellationToken, timeOutSeconds);
            }
            catch(Exception Ex)
        ...
        /// <param name="timeOutSeconds">Number of seconds before the http request times out.</param>
        /// <returns>the number of records deleted or (if instrument, event, and/or instance are provided) the number of items deleted over the total records specified.</returns>
        public async Task<string> DeleteRecordsAsync(string token, Content content, RedcapAction action, string[] records, int? arm, CancellationToken cancellationToken = default, long timeOutSeconds = 100, bool deleteLogging = false)
        {
            try
            {
                this.CheckToken(token);
        ...
        public async Task<string> ExportSurveyAccessCodeAsync(string token, string record, string instrument, string eventName, int repeatInstance, RedcapReturnFormat returnFormat = RedcapReturnFormat.json, CancellationToken cancellationToken = default, long timeOutSeconds = 100)
        {
            try
            {
                ...
                {
                    { "token", token },
                    { "content", Content.SurveyAccessCode.GetDisplayName() },
                    { "record", record },
                    { "instrument", instrument },
                    { "event", eventName },
                    { "repeat_instance", repeatInstance.ToString() },
        ...
        /// </remarks>
        /// <param name="token">The API token specific to your REDCap project and username (each token is unique to each user for each project). See the section on the left-hand menu for obtaining a token for a given project.</param>
        /// <param name="content">surveyAccessCode</param>
        /// <param name="record">the record ID. The name of the record in the project.</param>
        /// <param name="instrument">the unique instrument name as seen in the second column of the Data Dictionary. This instrument must be enabled as a survey in the project.</param>
        /// <param name="eventName">the unique event name (for longitudinal projects only).</param>
        /// <param name="repeatInstance">(only for projects with repeating instruments/events) The repeat instance number of the repeating event (if longitudinal) or the repeating instrument (if classic or longitudinal). Default value is '1'.</param>
        ...
        /// <param name="timeOutSeconds">Number of seconds before the http request times out.</param>
        /// <returns>Returns a unique survey participant identifier in plain text format for the specified record and instrument (and event, if longitudinal).</returns>
        public async Task<string> ExportSurveyAccessCodeAsync(string token, Content content, string record, string instrument, string eventName, int repeatInstance, RedcapReturnFormat returnFormat = RedcapReturnFormat.json, CancellationToken cancellationToken = default, long timeOutSeconds = 100)
        {
            try
            {
                this.CheckToken(token);
